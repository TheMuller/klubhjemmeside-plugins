  ...
  for($i = 1 ; $i < $limit ; $i++){
	...
	$owner_guid = $entity->owner_guid;
      if(vazco_tools::fileExists($file, $owner_guid)){
        $pics[$i] = elgg_view('vazco_newsletter/vazco_tools/output/icon',array('name' => $field['name'] , 'entity'=>$entity,'size'=>$size));
...
<div id="multi_image_<?php echo $name ?>" class="edit_image">
  	<p>
	  	...
	  	<?php
		if ($field['subtitle']){
			echo '<br><span class="v_subtitle">',$field['subtitle'],'</span>';                        
		}
		?>
	</p>
	<?php 
    foreach($pics as $key => $pic){
       ...
       echo "<img src='" . $pic . "'/>";
       echo "<p class='delete_link'><a href=\"#\" onClick=\"javascript:return vazco_myproject_deletePic_" . $name . "('" . $key . "')\"> " . elgg_echo("delete") . "</a></p>";
       echo "</div>";
